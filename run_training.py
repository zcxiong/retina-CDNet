...
if sys.platform=='win32':
    ...
    os.system('copy .\\model\\training.py .\\' +name_experiment+'\\'+name_experiment+'_training.py')
    os.system('copy .\\model\\get_NN_Model.py .\\' +name_experiment+'\\'+name_experiment+'_get_NN_Model.py')
else:
    os.system('cp configuration.txt ./' +name_experiment+'/'+name_experiment+'_configuration.txt')
    os.system('cp ./model/training.py ./' +name_experiment+'/'+name_experiment+'_training.py')
    os.system('cp ./model/get_NN_Model.py ./' +name_experiment+'/'+name_experiment+'_get_NN_Model.py')

# run the experiment
if nohup:
    print("\n2. Run the training on GPU with nohup")
    os.system(run_GPU +' nohup python -u ./model/training.py > ' +'./'+name_experiment+'/'+name_experiment+'_training.nohup')
else:
    print("\n2. Run the training on GPU (no nohup)")
    os.system(run_GPU +' python ./model/training.py  -s LOG_FILE=.\\' +name_experiment+ '\\' +'info.log')
